...
namespace WAS.Proxies
{
    public class LoggingProxy<T> : DispatchProxy
    {
        ...
        protected override object? Invoke(MethodInfo? targetMethod, object?[]? args)
        {
            ...
            try
            {
                _logger.LogInformation("--- [AOP ?úÏûë] {Message} ---", message);
                
                var result = targetMethod.Invoke(_target, args);

                // Task<TResult> ÎπÑÎèôÍ∏?Ï≤òÎ¶¨
                ...
                _logger.LogInformation("--- [AOP ?ÑÎ£å] {Message} ---", message);
                return result;
            }
            catch (TargetInvocationException ex)
            {
                var inner = ex.InnerException ?? ex;
                _logger.LogError(inner, "--- [AOP ?êÎü¨] {Message} : {Error} ---", message, inner.Message);
                throw inner;
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "--- [AOP ?êÎü¨] {Message} : {Error} ---", message, ex.Message);
                throw;
            }
        }
