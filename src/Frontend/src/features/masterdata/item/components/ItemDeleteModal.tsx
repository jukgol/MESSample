...
interface ItemDeleteModalProps {
  ...
  onConfirm: (id: number | string) => Promise<boolean>;
}

const ItemDeleteModal: React.FC<ItemDeleteModalProps> = ({ isOpen, onClose, itemId, onConfirm }) => {
  if (!isOpen || itemId === null) return null;

  const handleConfirm = async () => {
    const success = await onConfirm(itemId);
    if (success) {
      onClose();
    }
  };
    ...
    <div style={{
      ...
      <div className="premium-card" style={{ width: '400px', padding: '2rem', display: 'flex', flexDirection: 'column', gap: '1.5rem', background: '#1e1e24', border: '1px solid var(--border-color)', borderRadius: '12px', alignItems: 'center', textAlign: 'center' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
          <h2 style={{ margin: 0, fontSize: '1.4rem', color: 'white' }}>품목 삭제</h2>
          <p style={{ color: 'var(--text-secondary)', fontSize: '0.9rem', margin: 0, lineHeight: '1.5' }}>
            정말로 이 품목 정보를 삭제하시겠습니까?<br />
