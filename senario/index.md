...
```
[1. 공정 정의 (Process)] ➡️ [1-1. 생산 실패 케이스] ➡️ [2. 레시피 설계 (BOM)] ➡️ [3. 품목 도출 (Item)] ➡️ [4. 실물 입고 (Lot)] ➡️ [5. QC & 출하]
```
...
2. **[Step 1-1: 공정별 생산 실패 케이스 (Process Failure)](file:///f:/ProjectGit/MESSample/senario/step1_process_fail.md)**
   * 공정 가동 시 발생 가능한 비즈니스 예외 상황 및 실패 케이스들을 설계합니다.
3. **[Step 2: 레시피 설계 (BOM)](file:///f:/ProjectGit/MESSample/senario/step2_bom.md)**
   * 정의한 공정의 각 단계에 맞춰, 어떤 조합으로 라면이 조리/조립되는지 정의합니다.
4. **[Step 3: 품목 도출 (Item)](file:///f:/ProjectGit/MESSample/senario/step3_item.md)**
   * BOM 레시피를 실현하기 위해 우리 시스템이 인식하고 관리해야 하는 품목(Item) 목록을 도출합니다.
5. **[Step 4: 실물 입고 (Lot)](file:///f:/ProjectGit/MESSample/senario/step4_lot.md)**
   * 정의된 Item 정보를 바탕으로 실제 공장에 존재하는 자재의 물리적 묶음(Lot)들을 등록합니다.
6. **[Step 5: 품질 검사 (QC) 및 출하 (Shipment)](file:///f:/ProjectGit/MESSample/senario/step5_qc_shipment.md)**
